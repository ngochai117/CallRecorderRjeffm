...
import com.android.ictteam.callrecorderrjeffm.callrecorder.PhoneCallRecord;
import com.android.ictteam.callrecorderrjeffm.callrecorder.RecordingFragment;
import com.android.ictteam.callrecorderrjeffm.callrecorder.SettingsActivity;
import com.android.ictteam.callrecorderrjeffm.callrecorder.WhitelistActivity;
import com.android.ictteam.callrecorderrjeffm.database.CallLog;
import com.android.ictteam.callrecorderrjeffm.database.Database;
import com.android.ictteam.helpers.AboutDialog;
import com.android.ictteam.helpers.RateMeNowDialog;
...
public class MainActivity extends AppCompatActivity implements RecordingFragment.OnListFragmentInteractionListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener, MediaController.MediaPlayerControl {
    ...
    boolean permissionReadContacts,permissionRecord,permissionReadStorage,permissionWriteStorage,permissionReadPhoneState,permissionProcessCalls,permissionCapture;

    public void checkPermissions() {
        permissionRecord = ContextCompat.checkSelfPermission(this, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED;
        ...
        permissionProcessCalls = ContextCompat.checkSelfPermission(this, Manifest.permission.PROCESS_OUTGOING_CALLS) == PackageManager.PERMISSION_GRANTED;
        permissionReadStorage = ContextCompat.checkSelfPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED;
        permissionWriteStorage = ContextCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED;
        permissionReadContacts = ContextCompat.checkSelfPermission(this, Manifest.permission.READ_CONTACTS) == PackageManager.PERMISSION_GRANTED;
        permissionCapture = ContextCompat.checkSelfPermission(this, Manifest.permission.CAPTURE_AUDIO_OUTPUT) == PackageManager.PERMISSION_GRANTED;

        String sAlert="";
        List<String> arrayListString = new ArrayList<>();
        Log.d("permissionRecord",String.valueOf(permissionRecord));
        if(!permissionRecord) {
            Log.d("permissionRecord","false");
                Log.d("permissionRecord","shouldShowRequestPermissionRationale");
                sAlert += getString(R.string.str_access_permissions_contacts);
                arrayListString.add(Manifest.permission.RECORD_AUDIO);
        }
        if (!permissionReadPhoneState || !permissionProcessCalls){
                sAlert += getString(R.string.str_access_permissions_callphone);
                arrayListString.add(Manifest.permission.READ_PHONE_STATE);
                arrayListString.add(Manifest.permission.PROCESS_OUTGOING_CALLS);
        }
        if (!permissionReadStorage || !permissionWriteStorage){
                sAlert += getString(R.string.str_access_permissions_storage);
                arrayListString.add(Manifest.permission.READ_EXTERNAL_STORAGE);
                arrayListString.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);
        }
        if (!permissionReadContacts){
                sAlert += getString(R.string.str_access_permissions_contacts);
                arrayListString.add(Manifest.permission.READ_CONTACTS);
        }
        if (!permissionCapture){
            sAlert += getString(R.string.str_access_permissions_capture);
            arrayListString.add(Manifest.permission.CAPTURE_AUDIO_OUTPUT);
        }

        final String sPermissions[] = new String[arrayListString.size()];
        arrayListString.toArray(sPermissions);
        Log.d("sAlert",sAlert);
        Log.d("arrayListString",arrayListString.toString());
        Log.d("sPermissions",sPermissions.toString());

        if(!sAlert.isEmpty()){
            new android.app.AlertDialog.Builder(this).setCancelable(true).setTitle("Cấp quyền").setMessage(sAlert).setPositiveButton(R.string.dialog_ok, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    ActivityCompat.requestPermissions(MainActivity.this,sPermissions,0x01);
                }
            }).show();
        }
